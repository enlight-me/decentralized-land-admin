...
contract('LAParcelRegistry', function (accounts) {
  ...
  it("Should log an avent with the correct CSC when a new Feature is added", async () => {
    ...
    const result = await instance.claimParcel(dggsIndex1, wkbHash, addr1, label1, area, landUseCode1)
  ...
  it("Should emit a LogParcelClaimed event when a Parcel is Claimed", async () => {
    ...
    const tx = await instance.claimParcel(dggsIndex1, wkbHash, addr1, label1, area, landUseCode1);
  ...
  it("Shouldn't allow adding new parcel with the an existing dggsIndex", async () => {
    ...
    await instance.claimParcel(dggsIndex1, wkbHash, addr1, label1, area, landUseCode1);
    await truffleAssert.fails(instance.claimParcel(dggsIndex1, wkbHash, addr2, label2, area, landUseCode2),
      truffleAssert.ErrorType.REVERT);
  });

  it("Should set the correct values for the new claimed parcel", async () => {
    const result1 = await instance.claimParcel(dggsIndex1, wkbHash, addr1, label1, area, landUseCode1);
    const result2 = await instance.claimParcel(dggsIndex2, wkbHash, addr2, label2, area, landUseCode2);
  ...
  it("Should returns the correct address of the dggsIndex owner", async () => {
    await instance.claimParcel(dggsIndex1, wkbHash, addr1, label1, area, landUseCode1, { from: alice });
    assert.equal(await instance.dggsIndexOwner(dggsIndex1), alice, "Returned address do not correspond to the dggsIndex owner");

    await instance.claimParcel(dggsIndex2, wkbHash, addr1, label1, area, landUseCode1, { from: bob });
  ...
  it("Should returns the owner of the parcel", async () => {
    const result = await instance.claimParcel(dggsIndex1, wkbHash, addr1, label1, area, landUseCode1, { from: alice });
